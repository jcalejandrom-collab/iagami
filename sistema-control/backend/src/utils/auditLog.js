...
async function recordAuditLog(req, action, { userId = null, email = null, role = null, detail = '', severity = 'INFO' } = {}) {
  try {
    ...
    await query(
      `INSERT INTO audit_logs (action, user_id, email, role, ip, user_agent, detail, severity, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW() AT TIME ZONE 'UTC')`,
      [action, userId, email, role, ip, userAgent, String(detail).slice(0, 500), severity]
    );
  } catch (err) {
